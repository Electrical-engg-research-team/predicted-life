...
import org.apache.spark.ml.regression.{GeneralizedLinearRegression, LinearRegression, RandomForestRegressionModel, RandomForestRegressor}
...
object LRLife {
  ...
  def main(args: Array[String]): Unit = {
    ...
    //导入数据
    val rawdata = sc.textFile("hdfs://localhost:9000/user/hadoop/input/openingdata.txt")
    ...
    val Array(trainingDF,testDF) = vecDF.randomSplit(Array(0.8,0.2),12345)


  /*  //create LinearRegression Moder
    val lr1 = new LinearRegression()
    val lr2 = lr1.setFeaturesCol("features")
      .setLabelCol("Murder")
      .setFitIntercept(true)
    //RegParam:zhengzehua
    val lr3 = lr2.setMaxIter(10)
      .setRegParam(0.3)
...
*/
    ...
    tvModel.getEstimatorParamMaps.foreach{println}  //canshujihe
    tvModel.getEvaluator.extractParamMap()
    tvModel.getEvaluator.isLargerBetter
    tvModel.getTrainRatio
    tvModel.transform(testDF)
        .select("features","Murder","prediction")
        .show()
...
/*    //fit the model
    ...
    //print the cofficients and intercept for lr
    println(s"Coefficients:${lrModel.coefficients} /n Intercept:${lrModel.intercept}")
    ...
    println(s"RMES:${trainingSummary.rootMeanSquaredError}")
    println(s"r2:${//计算统计分类耗时
    val startTime = System.currentTimeMillis()
    println("startTime:"+startTime)
    val endTime = System.currentTimeMillis()
    println("endtime:"+endTime)
    val timeConsuming = endTime - startTime
    println("timeConsuming:"+timeConsuming)trainingSummary.r2}")
*/
//计算统计分类耗时
val startTime = System.currentTimeMillis()
    println("startTime:"+startTime)
